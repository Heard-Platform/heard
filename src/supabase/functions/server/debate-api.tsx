...
const compileRantsWithAI = async (rants: Rant[], topic: string): Promise<string[]> => {
  try {
...
Return ONLY the statements, one per line, without numbering or bullets.`;
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You are a skilled debate facilitator who helps structure discussions from raw input."
          },
          {
            role: "user",
            content: prompt
    ...
    const statements = generatedText
      .split('\n')
      .map(line => line.trim())
      .filter(line => line.length > 10) // Filter out empty lines or very short lines
      .slice(0, 30); // Ensure we don't exceed 30 statements

    console.log(`AI generated ${statements.length} statements from ${rants.length} rants`);
    return statements;
    ...
    try {
      ...
      if (room.rantFirst && room.phase === "lobby" && phase === "round1") {
        ...
        try {
          ...
          const aiStatements = await compileRantsWithAI(rants, room.topic);
          console.log(`AI generated ${aiStatements.length} statements`);

          // Create statement objects and save them
          const statementPromises = aiStatements.map(async (text, index) => {
            const statement: Statement = {
              id: generateId(),
              text: text,
          ...
          console.log(`Saved ${aiStatements.length} AI-compiled statements to database`);
