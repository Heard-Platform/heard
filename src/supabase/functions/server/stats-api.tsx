      ...
      for (const room of rooms) {
        ...
        if (t > since) {
          events.push({
            ...
            timestamp: t,
            id: room.id,
            label: room.topic || room.id,
            isAutoGenerated: SCRAPER_AUTHOR_IDS.has(room.hostId),
          });
        }
      }
      ...
      for (const statement of statements) {
        ...
        if (t > since) {
          ...
          events.push({
            ...
            isAutoGenerated: SCRAPER_AUTHOR_IDS.has(statement.author),
          });
        }
      }
      ...
      const dayCounts = Array.from({ length: 7 }, (_, i) => {
        ...
        for (const t of ALL_FEED_TYPES) {
          point[t] = events.filter(
            (e) => e.type === t && toDateKey(e.timestamp) === dayKey && !e.isAutoGenerated,
          ).length;
        }
        return point;
      });
