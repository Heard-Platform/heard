...
async function getNewPostsSince(cutoffTime: number): Promise<number> {
  try {
    const allRooms = await getAllRealDebates();
    return allRooms.filter(
      (room) => room.createdAt >= cutoffTime && !SCRAPER_AUTHOR_IDS.has(room.hostId)
    ).length;
  } catch (error) {
...
async function getNewStatementsSince(cutoffTime: number): Promise<number> {
  try {
    ...
    console.error("Error counting new statements:", error);
    return 0;
  }
}

async function getNewCommunitiesSince(cutoffTime: number): Promise<number> {
  try {
    const allCommunities = await getAllSubHeards<Community & { createdAt: number }>();
    return allCommunities.filter((community) => community.createdAt >= cutoffTime).length;
  } catch (error) {
    console.error("Error counting new communities:", error);
    return 0;
  }
}
