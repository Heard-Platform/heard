...
export function RealTimeResults({ statements, currentRound }: RealTimeResultsProps) {
  const [viewMode, setViewMode] = useState<'consensus' | 'categories' | 'clusters' | 'trends'>('consensus');
  ...
  const analysis = useMemo(() => {
    ...
    // Group by type
    const byType = {
      bridge: statements.filter(s => s.type === 'bridge').sort((a, b) => b.votes - a.votes),
      crux: statements.filter(s => s.type === 'crux').sort((a, b) => b.votes - a.votes),
      plurality: statements.filter(s => s.type === 'plurality').sort((a, b) => b.votes - a.votes),
      general: statements.filter(s => !s.type).sort((a, b) => b.votes - a.votes)
    };

    // Calculate consensus (statements with high positive votes)
    const consensus = byVotes.filter(s => s.votes >= 3);
    const controversial = statements.filter(s => s.votes === 0 || (s.votes > 0 && s.votes < 3));
    ...
    const clusters: ClusterData[] = [
      ...
      ...cluster,
      avgVotes: cluster.statements.length > 0 ? 
        cluster.statements.reduce((sum, s) => sum + s.votes, 0) / cluster.statements.length : 0,
      size: cluster.statements.length
    })).filter(cluster => cluster.size > 0);

    return { byVotes, byType, consensus, controversial, clusters };
  }, [statements]);

  const getTypeInfo = (type: string) => {
    switch (type) {
      case 'bridge': return { icon: Users, color: 'text-blue-600', bg: 'bg-blue-50', label: 'Bridges' };
      case 'crux': return { icon: Target, color: 'text-red-600', bg: 'bg-red-50', label: 'Cruxes' };
      case 'plurality': return { icon: Zap, color: 'text-purple-600', bg: 'bg-purple-50', label: 'Pluralities' };
      default: return { icon: Eye, color: 'text-gray-600', bg: 'bg-gray-50', label: 'General' };
    }
  };

  const StatementMini = ({ statement, showVotes = true }: { statement: Statement; showVotes?: boolean }) => (
    <motion.div
      layout
    ...
    >
      <div className="flex items-start justify-between mb-2">
        <span className="text-xs text-muted-foreground">@{statement.author}</span>
        ...
        <TabsContent value="clusters" className="space-y-4">
          {analysis.clusters.map(cluster => (
            <div key={cluster.id} className="p-3 border rounded-lg">
              ...
              <Progress value={(cluster.avgVotes / Math.max(...statements.map(s => s.votes)) || 1) * 100} className="mb-2" />
              <div className="space-y-2 max-h-24 overflow-y-auto">
                {cluster.statements.slice(0, 1).map(statement => (
                  <StatementMini key={statement.id} statement={statement} showVotes={false} />
                ))}
              </div>
            </div>
          ))}
        ...
        <TabsContent value="trends" className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="text-center p-4 border rounded-lg">
              ...
              <div className="text-xs text-muted-foreground">Statements with votes</div>
            </div>
            <div className="text-center p-4 border rounded-lg">
              <div className="text-2xl font-mono text-primary">
            ...
            <div className="text-center p-4 border rounded-lg">
              ...
              <div className="text-xs text-muted-foreground">Cruxes identified</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            {analysis.byVotes.slice(0, 3).map((statement, index) => (
              <div key={statement.id} className="flex items-center gap-3 p-2 border rounded">
