...
export function AchievementNotification({ achievement, onClose }: AchievementNotificationProps) {
  ...
  const getIcon = () => {
    switch (achievement.type) {
      case 'bridge': return <Users className="w-6 h-6 text-blue-500" />;
      case 'crux': return <Target className="w-6 h-6 text-red-500" />;
      case 'plurality': return <Zap className="w-6 h-6 text-purple-500" />;
  ...
  const getBgColor = () => {
    switch (achievement.type) {
      ...
      case 'crux': return 'bg-red-500';
      case 'plurality': return 'bg-purple-500';
