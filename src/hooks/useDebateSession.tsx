...
export function useDebateSession() {
  ...
  // Submit statement
  const submitStatement = useCallback(async (text: string, type?: 'bridge' | 'crux' | 'plurality') => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setUser(prev => prev ? {
          ...
          bridgePoints: type === 'bridge' ? prev.bridgePoints + response.data.pointsEarned : prev.bridgePoints,
          cruxPoints: type === 'crux' ? prev.cruxPoints + response.data.pointsEarned : prev.cruxPoints,
          pluralityPoints: type === 'plurality' ? prev.pluralityPoints + response.data.pointsEarned : prev.pluralityPoints,
  ...
  // Vote on statement
  const voteOnStatement = useCallback(async (statementId: string, voteType: 'up' | 'down') => {
  ...
  // Update room phase
  const updateRoomPhase = useCallback(async (phase: string) => {
    if (!user || !room) return false;
