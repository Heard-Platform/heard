...
interface DebateSessionContextType {
  ...
  markCoverCardSwiped: (roomId: string, coverType: "youtube" | "image") => Promise<void>;
...
export function DebateSessionProvider(
  ...
  const markCoverCardSwiped = useCallback(
    async (roomId: string, coverType: "youtube" | "image") => {
      try {
        const response = await api.markCoverCardSwiped(roomId, coverType);
        if (!response.success) {
