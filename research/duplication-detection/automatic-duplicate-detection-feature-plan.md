...
This complements the manual [statement deduplication feature](../../docs/planning/deduplication-feature-plan.md). The manual flow lets a host pick two statements and merge them by hand; this automatic flow catches duplicates that hosts miss or that arrive too quickly to triage.
...
`scripts/backfill-statement-embeddings.ts` (sibling to the existing extract/score/dryrun scripts in this directory):
...
This calibration was done once during the design phase (April 2026) against ~3,700 statements across ~750 rooms; 0.85 was the resulting recommendation. The code that performed it lives in this directory: three Deno scripts ([`scripts/extract-statements.ts`](scripts/extract-statements.ts), [`scripts/score-statement-similarity.ts`](scripts/score-statement-similarity.ts), [`scripts/dryrun-duplicate-detection.ts`](scripts/dryrun-duplicate-detection.ts)) and a self-contained React viewer (see [`README.md`](README.md) for setup). If those scripts still exist when this feature is revisited they can be re-run; if not, the design above describes what they need to do.
...
4. **Backfill script.** `scripts/backfill-statement-embeddings.ts` (in this directory). Idempotent. Run on staging first, then production.
